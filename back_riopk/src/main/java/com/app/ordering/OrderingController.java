...
public class OrderingController {
    ...
    @Secured({USER})
    @PostMapping
    public Result save(@RequestParam String type, @RequestParam String certId, @RequestParam(defaultValue = "") String email) {
        return new Result(
                true,
                StatusCode.SUCCESS,
                "Success Save",
                toDtoConverter.convert(service.save(type, certId, email))
        );
    }
